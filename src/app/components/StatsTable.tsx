...
function TableGroupHeader({ title, colSpan, bgClass, textClass }: TableGroupHeaderProps) {
  return (
    <th 
      colSpan={colSpan} 
      className={`p-2 border border-slate-600 text-center font-extrabold tracking-wider text-xs uppercase ${bgClass} ${textClass}`}
    >
      {title}
    </th>
  );
...
function SubHeader({ title, widthClass = '', extraClass = '' }: SubHeaderProps) {
  return (
    <th className={`p-3 border border-slate-600 text-center ${widthClass} ${extraClass}`}>
      {title}
    </th>
  );
}
